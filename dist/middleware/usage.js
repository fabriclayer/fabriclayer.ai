...
import { OVERAGE_COST_PER_REQUEST } from '../config.js';
...
function todayKey(accountId) {
    const d = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
    return `usage:${accountId}:${d}`;
}
async function usagePluginFn(app) {
    app.addHook('onRequest', async (request, reply) => {
        const path = request.url.split('?')[0];
        if (SKIP_PATHS.has(path) || FREE_PATHS.has(path))
            return;
        if (!request.account)
            return;
        ...
        try {
            ...
            if (count > dailyLimit) {
                const overage = await checkAndTrackOverage(accountId, plan, count);
                request.isOverage = true;
                request.overageCost = OVERAGE_COST_PER_REQUEST;
                if (!overage.allowed) {
    ...
    app.addHook('onResponse', async (request, reply) => {
        ...
        if (!request.account)
            return;
        const isOverage = request.isOverage === true;
