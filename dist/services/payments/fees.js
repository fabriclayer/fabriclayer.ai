 ...
 */
export async function calculateCosts(providerPrice, routingFeePct, useEstimatedGas = false) {
    ...
    if (useEstimatedGas) {
        try {
            ...
            const rawGas = await estimateGasCostUsd(130000n); // ~65k gas per transfer × 2
 ...
 */
export function calculateRoutingFee(providerCost, feePct) {
...
export function validatePaymentAmount(amount) {
    ...
    if (amount > 10000)
        return { valid: false, reason: 'Amount exceeds maximum ($10,000)' };
    if (!isFinite(amount))
        return { valid: false, reason: 'Amount must be finite' };
    return { valid: true };
}
function round6(n) {
