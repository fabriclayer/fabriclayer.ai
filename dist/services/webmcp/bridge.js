...
import { executeProvider } from '../routing/executor.js';
 ...
 */
export async function executeWebMCPTool(req, ctx) {
    ...
    if (provider.trustScore < 0.1) {
        throw new Error(`Provider trust score too low (${provider.trustScore.toFixed(2)}). ` +
            `Tool: ${tool.qualifiedName}`);
    }
    // ─── Free tool fast path ───
    if (tool.pricePerCall === 0) {
        const result = await executeFreeTool(tool, req.arguments, startTime);
        const txn = await recordTransaction(agent.id, ctx.accountId, provider.id, tool, result, startTime);
 ...
 */
export async function authoriseWebMCPExecution(req, ctx) {
    const tool = await getWebMCPTool(req.tool);
    if (!tool)
        throw new Error(`WebMCP tool not found: ${req.tool}`);
    const agent = await prisma.agent.findFirst({
    ...
    if (!provider)
        throw new Error(`Provider not found for tool`);
...
// ─── Helpers ───
async function executeFreeTool(tool, args, startTime) {
...
function buildToolEndpoint(tool) {
    // Convention: origin serves a WebMCP proxy at /.well-known/webmcp/execute
    return `${tool.origin}/.well-known/webmcp/execute`;
}
async function recordTransaction(agentId, accountId, providerId, tool, result, _startTime) {
