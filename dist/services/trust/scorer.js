 ...
 */
export function computeTrustScore(provider, weights = DEFAULT_WEIGHTS, feedbackAvg) {
    const signals = {};
    const penalties = [];
    // ─── 1. Success rate (0–1) ───
    ...
    if (provider.lastSeen) {
        const daysSinceActive = (Date.now() - new Date(provider.lastSeen).getTime()) / (24 * 60 * 60 * 1000);
