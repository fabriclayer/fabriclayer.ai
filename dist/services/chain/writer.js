...
const isDeployed = (addr) => addr !== ZERO_ADDR;
/**
 ...
 */
export async function registerAgentOnChain(name, category, endpoint) {
 ...
 */
export async function updateReputationOnChain(agentId, score, additionalInteractions) {
 ...
 */
export async function batchUpdateReputationOnChain(updates) {
 ...
 */
export async function mintIdentityOnChain(toAddress, registryId) {
 ...
 */
export async function getTotalAgentsOnChain() {
    if (!isDeployed(FABRIC_REGISTRY_ADDRESS))
        return null;
    try {
