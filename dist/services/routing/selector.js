...
import { DEFAULT_WEIGHTS, normaliseWeights } from '../trust/weights.js';
import { getCachedScores, setCachedScores } from '../cache/scores.js';
/**
 * Discover and score providers for a category.
 * Uses Redis cache on the hot path, falls back to DB + live scoring.
 */
export async function discoverAndScore(category, options) {
 ...
 */
export async function selectProvider(category, agentId, accountId, preferences, customWeights) {
    // ─── Discover and score ───
    ...
    let ranked = candidates.map((p) => {
        ...
        const prefBoost = preferences?.preferredProviders?.includes(p.id) ? 0.3 : 0;
        return {
            ...p,
            isFavorite: isFav,
...
// ─── Helpers ───
async function scoreProviders(providers, weights) {
 ...
 */
async function loadFeedbackAverages(providerIds) {
...
function buildSelectionReason(winner, hasFavorites, prefs) {
    const parts = [];
