 ...
 */
export async function isCircuitOpen(providerId) {
    try {
        ...
        if (info.state === 'open') {
            ...
            if (info.lastFailure && Date.now() - info.lastFailure > CIRCUIT_RESET_SEC * 1000) {
                await redis.set(key, JSON.stringify({ ...info, state: 'half-open' }), 'EX', CIRCUIT_RESET_SEC * 2);
 ...
 */
export async function recordFailure(providerId) {
    try {
        const key = `${CIRCUIT_PREFIX}${providerId}`;
        const data = await redis.get(key);
        const info = data
 ...
 */
export async function getCircuitInfo(providerId) {
    try {
        const key = `${CIRCUIT_PREFIX}${providerId}`;
        const data = await redis.get(key);
        if (!data)
 ...
 */
export async function filterHealthy(providers) {
 ...
 */
export async function selectFallback(candidates, excludeIds) {
