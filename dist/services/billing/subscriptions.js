...
import { STRIPE_PRICE_IDS, PLAN_CONFIG, } from '../../config.js';
// ─── Customer Management ───
/**
 * Get or create a Stripe customer for an account.
 */
export async function ensureStripeCustomer(accountId) {
 ...
 */
export async function createCheckoutSession(accountId, plan, successUrl, cancelUrl) {
    if (plan === 'FREE') {
 ...
 */
export async function changePlan(accountId, newPlan) {
    if (newPlan === 'FREE') {
