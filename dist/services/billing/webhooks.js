...
async function handleSubscriptionUpsert(sub) {
    ...
    if (!accountId)
        return;
    const plan = sub.metadata?.plan || 'BUILDER';
...
async function handleInvoicePaid(invoice) {
    ...
    await createInvoiceRecord(account.id, invoice.id, periodStart, periodEnd, subscriptionUsd, overageUsd, overageCount, routingFeesUsd, totalUsd, 'PAID');
...
async function handleInvoicePaymentFailed(invoice) {
    ...
    console.warn(`[Billing] Payment failed for account ${account.id} (${account.email})`);
}
