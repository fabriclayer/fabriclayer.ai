...
import { OVERAGE_COST_PER_REQUEST, STRIPE_OVERAGE_PRICE_ID, PLAN_CONFIG, } from '../../config.js';
 ...
 */
export async function checkAndTrackOverage(accountId, plan, currentCount) {
 ...
 */
export async function getDailyOverageCount(accountId) {
    const today = new Date().toISOString().split('T')[0];
    const key = `${OVERAGE_KEY_PREFIX}${accountId}:${today}`;
 ...
 */
export async function reportOverageToStripe(accountId, overageCount) {
    ...
    try {
        ...
        const overageItem = sub.items.data.find((item) => item.price.id === STRIPE_OVERAGE_PRICE_ID);
        if (!overageItem) {
        ...
        console.error('[Overage] Stripe reporting failed:', err.message);
        return { reported: false };
    }
}
/**
 ...
 */
export function calculateOverageCost(overageCount) {
 ...
 */
export async function getOverageSummary(accountId) {
    const account = await prisma.account.findUniqueOrThrow({
        where: { id: accountId },
    });
    const planConfig = PLAN_CONFIG[account.plan];
