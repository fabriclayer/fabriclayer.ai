import { z } from 'zod';
import { createCheckoutSession, createPortalSession, changePlan, cancelSubscription, reactivateSubscription, getSubscription, } from '../../services/billing/subscriptions.js';
...
export async function billingRoutes(app) {
    ...
    app.get('/billing/subscription', async (request) => {
        ...
        return {
            ...
            priceUsd: PLAN_PRICES_USD[request.account.plan] ?? 0,
            subscription: sub,
    ...
    app.post('/billing/checkout', async (request, reply) => {
        ...
        const session = await createCheckoutSession(request.account.id, plan, successUrl, cancelUrl);
    ...
    app.post('/billing/portal', async (request, reply) => {
        ...
        const session = await createPortalSession(request.account.id, returnUrl || 'https://dashboard.fabric.computer/settings');
    ...
    app.get('/billing/upcoming', async (request) => {
    ...
    // ─── Overage summary ───
    app.get('/billing/overage', async (request) => {
