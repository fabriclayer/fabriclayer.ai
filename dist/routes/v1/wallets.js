...
import { PlanFeatureError, ValidationError, NotFoundError, toErrorResponse, } from '../../utils/errors.js';
import { createManagedWallet, getWalletBalance, listAccountWallets, } from '../../services/payments/wallets.js';
...
export async function walletRoutes(app) {
    ...
    app.get('/wallets', async (request, reply) => {
        if (!request.account?.config.canRoute) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('managed wallets')));
        }
    ...
    app.post('/wallets', async (request, reply) => {
        ...
        if (!parsed.success) {
            return reply.status(400).send(toErrorResponse(new ValidationError('agentId is required')));
        }
    ...
    // ─── Get wallet balance ───
    app.get('/wallets/:agentId/balance', async (request, reply) => {
        if (!request.account?.config.canRoute) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('managed wallets')));
        }
