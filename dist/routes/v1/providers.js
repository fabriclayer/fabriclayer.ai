...
export async function providerRoutes(app) {
    ...
    app.post('/providers/register', async (request, reply) => {
        ...
        let onChainId = null;
        try {
            const chainResult = await registerAgentOnChain(body.name.trim(), body.category, body.endpoint.trim());
            if (chainResult) {
                ...
                if (body.walletAddress) {
                    try {
                        await mintIdentityOnChain(body.walletAddress.trim(), onChainId);
                    }
                    catch { }
                }
            }
    ...
    // ─── List providers ───
    app.get('/providers/list', async (request, reply) => {
