...
export async function favoritesRoutes(app) {
    // ─── List favorites for an agent ───
    app.get('/favorites/:agentId', async (request, reply) => {
        if (!request.account?.config.canFavorites) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('favorites')));
        }
