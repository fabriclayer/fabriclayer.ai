...
export async function evaluateRoutes(app) {
    app.get('/evaluate/:providerId', async (request, reply) => {
        const parsed = evaluateParamsSchema.safeParse(request.params);
        if (!parsed.success) {
            return reply.status(400).send(toErrorResponse(new ValidationError('Invalid provider ID')));
        }
