...
export async function budgetRoutes(app) {
    ...
    app.get('/budget', async (request, reply) => {
        if (!request.account?.config.canBudget) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('budget controls')));
        }
    ...
    // ─── Budget status ───
    app.get('/budget/:id/status', async (request, reply) => {
        if (!request.account?.config.canBudget) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('budget controls')));
        }
        ...
        if (!budget) {
            return reply.status(404).send(toErrorResponse(new NotFoundError('Budget')));
        }
