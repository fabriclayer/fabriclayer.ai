...
export async function webSearchRoutes(app) {
    app.post('/web-search', async (request, reply) => {
        if (!BRAVE_API_KEY) {
            return reply.status(503).send(toErrorResponse(new Error('Web search is not configured')));
        }
        ...
        try {
            ...
            return reply.status(502).send(toErrorResponse(new Error('Search request failed')));
        }
        if (!response.ok) {
            request.log.error({ status: response.status }, 'Brave Search returned error');
            return reply.status(502).send(toErrorResponse(new Error('Search upstream error')));
        }
