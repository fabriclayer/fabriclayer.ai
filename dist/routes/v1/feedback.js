...
export async function feedbackRoutes(app) {
    app.post('/feedback', async (request, reply) => {
        if (!request.account?.config.canFeedback) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('feedback')));
        }
        ...
        if (!transaction) {
            return reply.status(404).send(toErrorResponse(new NotFoundError('Transaction')));
        }
        ...
        await queueReputationUpdate(transaction.providerId, null, score);
        return {
            id: feedback.id,
            transactionId,
            score,
