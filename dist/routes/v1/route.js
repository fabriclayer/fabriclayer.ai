...
import { PlanFeatureError, ValidationError, NotFoundError, BudgetExceededError, ProviderError, toErrorResponse, } from '../../utils/errors.js';
...
export async function routeRoutes(app) {
    app.post('/route', async (request, reply) => {
        ...
        if (!request.account?.config.canRoute) {
            return reply.status(403).send(toErrorResponse(new PlanFeatureError('routing')));
        }
        // ─── Validate body ───
        const parsed = routeBodySchema.safeParse(request.body);
        if (!parsed.success) {
            return reply.status(400).send(toErrorResponse(new ValidationError(parsed.error.issues[0].message)));
        }
        ...
        if (budgetId) {
            ...
            if (budget && budget.hardCap && budget.spentUsd >= budget.limitUsd) {
                return reply.status(402).send(toErrorResponse(new BudgetExceededError(budgetId)));
            }
        }
        // ─── Select provider ───
        const customWeights = request.account.config.customWeights
            ? request.body?.trustWeights
            : undefined;
        const selection = await selectProvider(category, agentId, request.account.id, preferences, customWeights);
        if (!selection) {
            return reply.status(404).send(toErrorResponse(new NotFoundError(`No providers found for category: ${category}`)));
        }
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                request.log.warn({ providerId: provider.id, attempt, error: err.message }, 'Provider execution failed, trying fallback');
                continue;
            }
        }
        increment('routes.exhausted');
        return reply.status(502).send(toErrorResponse(new ProviderError(`All providers failed after ${triedIds.size} attempts. Last error: ${lastError?.message ?? 'unknown'}`)));
    });
}
