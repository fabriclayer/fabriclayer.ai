 ...
 */
export const FABRIC_MCP_TOOLS = [
    {
        name: 'fabric_discover',
        description: 'Find AI service providers by category. Returns a ranked list scored by trust, reliability, and price. Use this to explore what providers are available before routing a request.',
        inputSchema: {
            type: 'object',
            properties: {
                category: {
                    type: 'string',
                    description: 'Service category to search (e.g. "image-generation", "code-review", "translation", "transcription", "data-analysis")',
                },
                limit: {
                    type: 'number',
    ...
    {
        name: 'fabric_route',
        description: 'Route a request to the best available provider. Fabric selects the optimal provider based on trust scores, your preferences, and favorites — then executes the request and handles payment via x402 on Base L2. Returns the provider response and a payment receipt.',
        inputSchema: {
            type: 'object',
            properties: {
                agentId: {
                ...
                input: {
                    type: 'object',
                    description: 'Request payload to send to the provider (shape depends on the category/provider)',
                },
                preferences: {
                    type: 'object',
    ...
    },
    {
        ...
        description: 'Get a detailed trust profile for a specific provider. Includes trust score breakdown (7 signals), recent feedback, latency stats (p50/p95/p99), circuit breaker status, and transaction history. Use this to inspect a provider before routing to it.',
        inputSchema: {
            type: 'object',
            properties: {
                providerId: {
    ...
    },
    {
        ...
        description: 'Submit a quality rating after a routed request completes. Feedback improves trust scores for future routing. Score from 1 (terrible) to 5 (excellent). Optionally tag and comment.',
        inputSchema: {
            type: 'object',
            properties: {
                transactionId: {
                ...
                tags: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Optional tags (e.g. "fast", "accurate", "slow", "wrong-output")',
                },
                comment: {
                    type: 'string',
    ...
    {
        name: 'fabric_budget',
        description: 'Manage spend controls. List existing budgets or create a new one. Budgets can be account-level or agent-level, with daily/weekly/monthly periods and optional hard caps that block requests when exceeded.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
    ...
    },
    {
        ...
        description: 'Discover WebMCP tools — browser-native tool contracts registered by websites via the W3C WebMCP spec (navigator.modelContext). These tools run client-side in the browser with Fabric trust scoring and x402 payment wrapping. Filter by category, origin, price, or trust score.',
        inputSchema: {
            type: 'object',
            properties: {
                category: {
    ...
    },
    {
        ...
        description: 'Execute a WebMCP tool through Fabric\'s trust and payment layer. Resolves the tool, verifies the provider\'s trust score, handles x402 payment settlement on Base, and returns the result. Use fabric_webmcp_discover first to find available tools.',
        inputSchema: {
            type: 'object',
            properties: {
                tool: {
    ...
    },
    {
        ...
        description: 'Register WebMCP tool contracts from a website origin. Sites publish tools via navigator.modelContext.registerTool() — this indexes them in Fabric\'s registry with trust scoring and payment support.',
        inputSchema: {
            type: 'object',
            properties: {
                origin: {
    ...
    },
    {
        ...
        description: 'Manage preferred providers. Favorited providers get a ranking boost during routing. Higher priority = bigger boost.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
