...
import { recordSuccess } from '../../services/routing/fallback.js';
...
import { PlanFeatureError, NotFoundError, } from '../../utils/errors.js';
import { increment } from '../../utils/metrics.js';
import { discoverWebMCPTools, registerWebMCPTools, } from '../../services/webmcp/registry.js';
import { executeWebMCPTool, } from '../../services/webmcp/bridge.js';
export async function mcpRoutes(app) {
    ...
    // ─── Tool execution ───
    app.post('/mcp/execute', async (request, reply) => {
...
async function handleRoute(args, request) {
    ...
    const selection = await selectProvider(args.category, args.agentId, request.account.id, args.preferences);
    if (!selection)
        throw new NotFoundError(`No providers for: ${args.category}`);
...
async function handleFeedback(args, request) {
    ...
    await queueReputationUpdate(transaction.providerId, null, args.score);
    return {
        content: [
            {
                type: 'text',
...
async function handleBudget(args, request) {
...
async function handleFavorites(args, request) {
...
// ─── WebMCP Handlers ───
async function handleWebMCPDiscover(args) {
...
async function handleWebMCPExecute(args, request) {
    if (!request.account?.config.canRoute) {
        throw new PlanFeatureError('routing');
    }
...
async function handleWebMCPRegister(args, request) {
